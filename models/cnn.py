...
def cnn(csv_file):
    ...
    new_model = keras.models.load_model("models/cnn.h5")
    ...
    # Normalize values
    values = df.values
    normalized = pd.DataFrame(normalize(values)).to_numpy()
    
    # Reshape input for CNN: (samples, height=1, width=features, channels=1)
    num_samples, num_features = normalized.shape
    normalized = normalized.reshape((num_samples, 1, num_features, 1))
